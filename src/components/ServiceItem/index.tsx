...
const InfoBar = ({ title, desc }: { title: string; desc: string }) => {
  return (
    <div className="flex mb-[3px]">
      <p className="text-gray10 dark:text-white font-inter font-bold text-xs min-w-[130px]">
        {title}:{" "}
      </p>
      <p className="text-xs font-medium text-gray10 dark:text-white font-inter">
        {desc}
      </p>
    </div>
  );
...
const ServiceItem: React.FC<ServiceItemProps> = ({
  ...
  return (
    <div
      key={id}
      className="flex w-full h-[250px] bg-white dark:bg-gray10 rounded-[10px] drop-shadow-[2px_2px_4px_#00000026] p-6 mb-4 items-center justify-between "
    >
      <div className="flex flex-col w-full h-full">
        <h3 className="text-gray10 dark:text-white font-inter font-bold text-base mb-[10px]">
          {name}
        </h3>

        <div className="flex items-center justify-between w-full h-full">
          <div className="flex flex-col">
            <InfoBar title={t("servicePage.card.country")} desc={region} />

            {city && <InfoBar title={t("servicePage.card.city")} desc={city} />}
            <InfoBar title={t("servicePage.card.address")} desc={address} />

            <InfoBar title={t("servicePage.card.phone")} desc={phone[0]} />
            <InfoBar
              title={t("servicePage.card.mail")}
              desc={mail.join(", ")}
            />
            <InfoBar title={t("servicePage.card.web")} desc={web} />
          </div>

          {isImage && service && (
            <div className="flex items-center h-full gap-4 dark:items-start">
              {service.map((service, index) => (
                <div
                  key={index}
                  className="w-[141px] h-[141px] flex flex-col items-center justify-start border-[1px] dark:border-[0px] border-gray2 rounded-[10px]"
                >
                  <img
                    ...
                    alt={service}
                  />
                  <p className="font-inter font-bold text-[12px] text-gray10 dark:text-white">
                    {service == "Exc"
                      ? "Ekskavatör"
                      : service == "Tele"
                      ? "Telehandler"
